...
class Mul:
    ...
    def evaluate(self, x_value):
        ...
        left = self.p2.evaluate(x_value)
        right = self.p1.evaluate(x_value)
        return Int(left.i * right.i)
...
class Sub:
    ...
    def __repr__(self):
        ...
        left_str = f"( {repr(self.p1)} )" if isinstance(self.p1, (Add, Sub)) else repr(self.p1)
        right_str = f"( {repr(self.p2)} )" if isinstance(self.p2, (Add, Sub)) else repr(self.p2)
...
class Div:
    ...
    def __repr__(self):
        ...
        left_str = f"( {repr(self.p1)} )" if isinstance(self.p1, (Add, Sub, Div)) else repr(self.p1)
        right_str = f"( {repr(self.p2)} )" if isinstance(self.p2, (Add, Sub, Div)) else repr(self.p2)
        return left_str + " / " + right_str
